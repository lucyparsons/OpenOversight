...
def upgrade():
    ...
    op.create_table(
        ...
        sa.UniqueConstraint('job_title', 'department_id', name='unique_department_job_titles'),
        sa.UniqueConstraint('order', 'department_id', name='unique_department_job_order'))
    op.create_index(op.f('ix_jobs_is_sworn_officer'), 'jobs', ['is_sworn_officer'], unique=False)
...
def downgrade():
    ...
    op.add_column('assignments', sa.Column('rank', sa.VARCHAR(length=120), autoincrement=False, nullable=True))

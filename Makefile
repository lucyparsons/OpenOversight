...
test: start  ## Run tests
	if [ -z "$(name)" ]; then \
		FLASK_ENV=testing docker-compose run --rm web pytest --cov=OpenOversight --doctest-modules -n auto --dist=loadfile -v OpenOversight/tests/; \
	else \
	    FLASK_ENV=testing docker-compose run --rm web pytest --cov=OpenOversight --doctest-modules -v OpenOversight/tests/ -k $(name); \
	fi
